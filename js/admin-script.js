...
import { getFirestore, collection, addDoc, serverTimestamp, doc, getDoc, onSnapshot, query, orderBy, updateDoc } from 'https://www.gstatic.com/firebasejs/12.0.0/firebase-firestore.js';
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (docSnap.exists() && docSnap.data().isAdmin) {
            ...
            // If user is not admin, sign them out from admin page and redirect
            await signOut(auth);
...
function initializeAdminPanel() {
    setupTabs();
    // Site Management Listeners
    listenForUsers();
    listenForInquiries();
    fileUploadForm.addEventListener('submit', handleFileUpload);
    // Investor Outreach Listeners
    listenForInvestors();
...
userListContainer.addEventListener('change', async (e) => {
    if (e.target.classList.contains('admin-toggle')) {
        ...
        try {
            ...
            e.target.checked = !newAdminStatus; // Revert checkbox on failure
        }
    }
});
...
function handleFileUpload(e) {
    ...
    const storageRef = ref(storage, `investor-documents/${file.name}`);
    ...
    uploadTask.on('state_changed', 
        ...
        (error) => {
            ...
            uploadStatus.textContent = `Upload failed: ${error.code}`;
        }, 
        async () => {
            uploadStatus.textContent = 'Upload complete! Getting download link...';
            try {
                ...
                uploadStatus.textContent = `Success! ${file.name} is now available to investors.`;
                ...
                console.error("Could not save file data:", error);
                uploadStatus.textContent = 'Upload failed during finalization.';
            }
        }
    );
}
...
async function handleAddInvestor(e) {
    ...
    investorData.contacted = false; // Default value
